...
// Helper utility for downloading files
...
// Action: Fetch code, copy to clipboard, trigger main download, and handle extra attachments
async function handleAction(id) {
  const actualPath = `programs/program${id}.py`;
  let copied = false;

  try {
    // 1. Fetch code dynamically
    const response = await fetch(actualPath);
    if (!response.ok) throw new Error("Failed to load file content");
    const codeText = await response.text();

    // 2. Copy to clipboard
    copied = await copyToClipboard(codeText);
  } catch (err) {
    console.warn("Fetch/Copy failed (likely offline or file:// protocol). Proceeding with downloads only.", err);
  }

  // 3. Trigger main download (Runs regardless of fetch success or failure)
  downloadFile(actualPath, `program${id}.py`);

  // 4. Handle extra asset downloads based on the ID
  if (id === 8) {
  ...
  // 5. Present the toast alert message based on clipboard state
  if (copied) {
